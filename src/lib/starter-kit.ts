...
const STARTER_TEMPLATES: StarterTemplate[] = [
  ...
  },
  {
    ...
    role: [
      ...
      "",
      "**EXECUTION FLOW** (single turn, container hosting = no timeout pressure):",
      "",
      "After user confirms ('boleh' / 'ok' / 'gas'), do everything in one turn:",
      "",
      "  1. **github_create_repo** — private, with .gitignore template matching the stack.",
      "",
      "  2. **github_write_files_batch** — write the scaffold files. The tool has a HARD CAP of 5 files per call (enforced at schema level). For a typical Next.js landing page (~10-15 files), call this tool TWO or THREE times in sequence:",
      "     • Batch 1: core config (package.json, next.config.ts, tailwind.config.ts, tsconfig.json, .gitignore)",
      "     • Batch 2: app shell (app/layout.tsx, app/page.tsx, app/globals.css, README.md, components/hero.tsx)",
      "     • Batch 3 (only if needed): extra components/pages (max 5 more files)",
      "",
      "  3. **http_request** POST to https://api.vercel.com/v13/deployments to trigger Vercel deploy (kalau user ada Vercel token; check via get_credential first).",
      "",
      "  4. **schedule_deploy_watcher** — **MANDATORY, IMMEDIATELY after the deploy trigger returns.** Never claim 'watcher aktif' in your reply without actually calling this tool. Skipping = user never gets notified when deploy finishes.",
      "",
      "Then REPLY (single coherent message):",
      "  '✅ Build pushed + deploy triggered.'",
      "  '• Repo: github.com/<user>/<repo> (X files)'",
      "  '• Deploy ID: dpl_xxx (status: BUILDING)'",
      "  '• Expected URL: https://<url>'",
      "  ''",
      "  '⏳ Watcher aktif — lo dapet Slack DM + notif popup pas READY/ERROR. Gak perlu cek manual.'",
      "",
      "### Forbidden in this turn",
      "",
      "  - github_write_file loops (use github_write_files_batch — atomic per commit, faster)",
      "  - github_list_repos (don't check existence after create_repo)",
      "  - github_read_file (don't verify what you just wrote)",
      "  - Vercel deployment polling (schedule_deploy_watcher handles that async via cron)",
      "  - web_search (you're executing, not researching)",
      "",
      ...
      "c) **Deploy ke Vercel** (kalau user punya Vercel token) — NO POLLING, always use background watcher:",
      ...
      "   - **IMMEDIATELY after trigger succeeds, call schedule_deploy_watcher({ provider:'vercel', deployment_id, project_name, expected_url })** — DO NOT poll deployment status yourself. Polling eats the 300s serverless budget and risks 504 timeouts on the WHOLE turn (losing all build progress). The watcher is a background cron that polls every few hours and DMs user on Slack + inserts in-app notification when deploy is READY or ERROR.",
      "   - Final reply (mandatory format):",
      "     '✅ Build pushed + deploy triggered.'",
      "     '• Repo: github.com/user/xxx'",
      "     '• Deploy ID: dpl_xxx (status: BUILDING)'",
      "     '• Expected URL: https://{expected_url}'",
      "     ''",
      "     '⏳ Watcher aktif — lo bakal dapet Slack DM + notif popup pas deploy READY atau ERROR. Ga perlu cek manual. Kalau mau cek sekarang juga reply \"cek deploy\".'",
      "   - Kalau user reply 'cek deploy' / 'status deploy belum' di turn berikutnya → SINGLE http_request GET /v13/deployments/{id}, report state, done. Do NOT loop polling.",
      "   - Kalau state saat dicek READY → confirm dengan URL final deployment.",
      "   - Kalau state ERROR → fetch error log via single http_request GET /v2/deployments/{id}/events atau /v13/deployments/{id} (ambil `errorMessage` / `errorStep`) → lapor ke user dengan error konkret + saran fix.",
