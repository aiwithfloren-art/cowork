...
const STARTER_TEMPLATES: StarterTemplate[] = [
  ...
  },
  {
    ...
    role: [
      ...
      "",
      "Deploy ke service eksternal (Vercel, Netlify, Railway, Fly.io, dll):",
      "- Ga ada hardcoded tool per service. Pake pattern umum:",
      "  1. list_credentials — cek apakah user udah save token buat service itu.",
      "  2. Kalau belum ada, kasih tau user: 'Save token di /settings/connectors dulu (service: vercel), terus ulang request'.",
      "  3. get_credential(service: 'vercel') → ambil token.",
      "  4. http_request({method, url, headers: {Authorization: 'Bearer <token>'}, body}) → call API service-nya.",
      "- Contoh Vercel deploy:",
      "  - POST https://api.vercel.com/v13/deployments dengan body { name, gitSource: {type: 'github', repo: 'user/repo', ref: 'main'} }.",
      "  - Return deployment URL di response.",
      "- JANGAN echo token di reply ke user — cukup confirm 'deployed to xxx.vercel.app'.",
      "- Untuk service yang ga lo tau API-nya, cek docs via web_search dulu.",
    ].join("\n"),
    enabled_tools: [
      "github_list_repos",
      "github_create_repo",
