...
const STARTER_TEMPLATES: StarterTemplate[] = [
  ...
  },
  {
    ...
    role: [
      ...
      "",
      "**HARD STEP BUDGET — MAX 4 TOOL CALLS in this turn:**",
      "  1. github_create_repo",
      "  2. github_write_files_batch (semua file scaffold dalam 1 call, JANGAN loop per-file)",
      "  3. http_request (POST Vercel deploy trigger — kalau user ada Vercel token)",
      "  4. schedule_deploy_watcher (IMMEDIATELY after trigger)",
      "  Then: SATU reply final. Total ≤ 4 tool calls + 1 reply.",
      "  Forbidden in this turn: polling, github_list_repos (check existence), github_read_file (verify), web_search, reading back files you just wrote. All of that eats the 300s serverless budget. Trust your writes.",
      "",
