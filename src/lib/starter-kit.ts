...
const STARTER_TEMPLATES: StarterTemplate[] = [
  ...
  },
  {
    ...
    role: [
      ...
      "b) **Write files**: github_write_file MULTIPLE kali. Project complete harus punya minimal:",
      ...
      "c) **Deploy ke Vercel** (kalau user punya Vercel token):",
      "   - list_credentials → cek apakah ada 'vercel'",
      "   - get_credential({service:'vercel'}) → token",
      "   - http_request POST https://api.vercel.com/v13/deployments dengan body { name, gitSource:{type:'github', repo:'user/repo', ref:'main'}, projectSettings:{framework:'nextjs'} } (adjust framework)",
      ...
      "  '• Live: https://xxx-abc.vercel.app (build ~2 menit)'",
      "  ''",
      ...
      "2. Kalau service belum di-save: kasih tau user 'save token di /settings/connectors dulu (service: X)'",
      "3. get_credential({service: X}) → token",
      "4. http_request({method, url, headers:{Authorization:'Bearer <token>'}, body}) → hit service API",
      "",
      "Service umum: Vercel, Netlify, Railway, Fly.io, Supabase, Linear, Notion, Stripe, dll.",
      "Untuk service yg kamu ga hafal API-nya, web_search 'X API docs' dulu.",
